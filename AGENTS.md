...
## 第十四條 SOP — 情緒動詞黑名單 + 品牌詞庫（Copy Library Lock）

文案生成時必須強制 grep 黑名單，避免落入「裝飾性空洞」。品牌詞庫定義可複用句式與禁用詞。

### A. 情緒動詞黑名單（禁用詞）

**絕對禁用**（情緒動詞、空泛承諾、無法驗證）：
- 負責到底 / 用心 / 貼心 / 真心 / 耐心 / 細心 / 放心
- 全力以赴 / 使命必達 / 不負所托
- 完美 / 完美融合 / 完美呈現 / 完美替代
- 絕對 / 最好 / 最優 / 最棒 / 最佳 / 首選 / 唯一
- 頂尖 / 一流 / 完善 / 全方位 / 量身打造 / 優質 / 高品質
- 打造夢想 / 圓夢 / 成就

**優先改寫方向**（動作動詞 > 情緒動詞）：
- 「用心傾聽」→「先聽完再開口」/「先收完需求再談風格」
- 「負責到底」→「12 個月內主動回報與修復」/「保固期內主動回訪」
- 「完美融合」→「把 X 整合進 Y」/「讓 X 與 Y 同步運作」
- 「全方位」→「分階段處理」/「依階段交付」

**豁免清單**（品牌功能命名 / 系統軌跡名稱）：
- 「安心安排」= renovation-process 時間軸「保守估算」軌道名稱（技術命名）
- 「安心守護包」= smart-home-quiz 結果包的產品名稱（產品線命名）
- 「安心守護」/「安心感」= 該產品線情境描述用語（產品線延伸用語）
- 「先安靜看清楚」= faq hero h1 修辭用語（指「不催促」的語氣，非情緒動詞）

### B. 品牌詞庫（可複用句式）

**CTA / 按鈕**：
- 「把 X 講清楚」→「把需求講清楚」/「把空間條件與生活感說清楚」/「把流程、排程與隱蔽工程講清楚」
- 「下一步」= 統一引導詞，所有 CTA 區副標統一用「下一步」
- 「送出 X」/「預約 X」= 表單提交按鈕標準用語

**Hero / Section h2 起手式**：
- 「先 X，再 Y」= 引導式句型（先把需求講清楚 / 先把流程講清楚 / 先把空間說清楚）
- 「不是 X，而是 Y」= 對比式句型（不是堆設備，而是把 HA、NAS 與空間一起整合好）
- 「如果 X，就 Y」= 條件式句型（如果方向已經對了，就用最舒服的方式開始）
- 「特別適合 X」= 受眾錨定句型（特別適合正在裝修前期的人）
...
**h1 / h2 / h3 內禁用**：
- 句尾句號「。」= 中文書寫習慣，標題不適用
- 句中逗號「，」= 機械性串接兩個獨立子句（除非是有意識的對比句式「X，不是 Y」）

**豁免對象**（品牌語法）：
- 「不只看圖，也看現場」= 「X，也 Y」並列結構
- 「不只是裝設備，而是把 HA、NAS 與空間一起整合好」= 「不是 X，而是 Y」對比結構
- 「如果方向已經對了，就用最舒服的方式開始」= 「如果 X，就 Y」條件結構
- 「先完成 5 題，青曦再幫你抓最適合的智能家居方向」= 「先 X，再 Y」引導結構

### D. 驗證命令

```bash
# 情緒動詞黑名單（前台頁面，排除豁免的後台功能命名）
grep -rEn "負責到底|用心|貼心|真心|耐心|細心|放心|全力以赴|使命必達|完美融合|絕對|最優|首選|量身打造|全方位|優質" src/pages/ src/components/ src/layouts/ | grep -v "/blog/" | grep -v "安心安排\|安心守護"

# h1/h2/h3 多餘標點
grep -rEn "<h[1-3][^>]*>[^<]*[，。！？：；][^<]*</h[1-3]>" src/pages/ src/components/ src/layouts/

# 空泛形容詞掃描
grep -rEn "優質|專業|頂尖|一流|完善|全方位|量身打造|高品質|最優|卓越|頂級|首選|唯一|完美" src/pages/ src/components/ src/layouts/ | grep -v "/blog/"
```

### E. 第 20 輪執行紀錄

首次落地：移除後台 7 個頁面（contact / faq / standard-sop / smart-home-quiz / requirement-form）的 h1/h2 句尾句號共 14 處；建立本附錄作為下次新增文案時的可 grep 文件。原本散落在「責任到底」「用心傾聽」「完美融合」三處的情緒動詞已於第 20 輪前清除。

### F. 第 21 輪擴展：portfolio 與 thank-you 頁覆蓋

本輪補掃三個先前被遺漏的頁面：
- `portfolio/xizhi-zhongxing-rd.astro:275` h2 「喜歡這樣的空間方向嗎？」 → 刪尾問號
- `portfolio/index.astro:845` h2 「如果你已經看出偏好，下一步就直接講需求。」 → 刪尾句號
- `consultation-thank-you.astro:13` h1 「需求已送出，接下來交給青曦。」 → 刪尾句號

**重要豁免**：`src/pages/blog/**/*.astro` 的 h2/h3 句尾標點（含問號、句號、驚嘆號）**不在本 SOP 範圍內**。理由：
- blog 是「長文文章」，h2/h3 是文章章節標題（如「一、天花板：先判斷你在解決什麼問題」），屬中文寫作慣例
- blog 文案受「內容鎖定條款」保護（不得增刪改寫），語法結構不可動
- 第八條 SOP 規範對象是「hero h1 / section h2 / CTA / eyebrow / tag」等 UI 元件，**非長文文章內的小節標題**

### G. 第 21 輪豁免邏輯

掃描後確認下列標點屬「有意識的對比句式」，不違反本 SOP：
- 「按場景切換，不再全部堆在同一頁」= 「不再 Y」反向對比結構
