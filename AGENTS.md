...
違規衝突處理流程 (Conflict Resolution Protocol)：當發現「優化修改項目」與本憲法的 SOP 或設計原則牴觸時，**禁止 AI 自行決定直接修代碼**，必須依下列流程處理：
  - **判斷優先級**：
    * 違規屬於「明確違規」（如字級 ≥ 3rem × 字距 ≥ 0.05em 字島）→ 必須修代碼，不可修憲
    * 違規屬於「設計哲學衝突」（如「Apple 極簡」 vs 使用者要求「繽紛色彩」）→ 必須先修憲再修代碼
    * 違規屬於「邊界值」（如行高 1.24 對字級 2.7rem）→ 必須詢問使用者
  - **推薦選項輸出格式**：當發現違規時，AI 必須輸出以下三選一結構：
    ```
    偵測到違規：[具體違規值] vs [憲法條款]
